...
def create_and_publish_datawrapper_chart(df, team1_abbr, team2_abbr):
    ...
    try:
        ...
        if chart_id:
            st.subheader("Datawrapper Chart") # Kept this subheader as it's for the embedded chart
...
"""
            chart_id_string = f"{chart_id}"

            full_responsive_embed_code = embed_code_placeholder.replace("CHART_ID",chart_id_string)

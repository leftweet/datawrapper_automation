...
def create_and_publish_datawrapper_chart(df, team1_abbr, team2_abbr):
    ...
    st.info("Creating Datawrapper chart...")
    ...
    try:
        ...
        st.success(f"PBP data saved to temporary file: {csv_filename}")
        ...
        st.success(f"Chart created with ID: {chart_id}")

        # Step 3: Upload CSV data
        st.info("Uploading data to Datawrapper...")
        ...
        st.info("Patching chart metadata...")
        ...
        st.success("Metadata patched.")

        # Step 5: Publish chart
        st.info("Publishing chart...")
